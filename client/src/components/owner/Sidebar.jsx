  ...
  return (
    ...
    max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
        ...
        <div className='group relative'>
            <label htmlFor="image">
                <img src={image ? URL.createObjectURL(image) : user?.image || "https://i.pinimg.com/736x/9f/e1/df/9fe1dfbc2d108cbc06a5ee0a75858023.jpg"} alt="" 
                className='h-9 md:h-14 w-9 md:w-14 rounded-full mx-auto'/>
                <input type="file" id='image' accept="image/*" hidden onChange={e=>setImage(e.target.files[0])}/>

                <div className='absolute hidden top-8 right-0 left-0 bottom-0
    ...
    <div className='w-full'>
        {ownerMenuLinks.map((link, index)=>(
            ...
            items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path === location.pathname ? 'bg-primary/10 text-primary' : 'text-gray-600'}`}>
                <img src={link.path === location.pathname ? link.coloredIcon : link.icon} alt="vehicle icon" />
